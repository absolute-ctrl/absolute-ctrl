...
- `logo.row_seconds`, `logo.bar_seconds`, `logo.hold_seconds`: one row of the splash logo, its fill bar, and the pause before the BIOS takes over.
...
The screen that plays *before* the BIOS, like a hardware vendor logo. Delete the whole `logo` block, or set `"enabled": false`, and the profile starts at the BIOS as before.
