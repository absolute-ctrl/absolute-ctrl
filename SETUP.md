...
- `art`: `terrain`, `terminal`, `http_log`, `voxels`, `engine` or `idle`
...
- `http_log.header`, `http_log.columns`, `http_log.step_seconds`: same scrolling-log mechanic as `terminal`, styled as an HTTP access log instead of mainframe job output
...
- `telemetry.languages_ignore`: language names (case-insensitive) to drop from the mix before picking the top `languages_shown`, e.g. `["HTML", "CSS"]`
