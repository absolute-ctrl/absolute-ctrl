...
Raise `low` to strip more background; lower it to keep dark clothing and hair. With `cutout` on, set `fade_edges` to `0` -- the soft edge comes from the subject now, not from the crop box.
