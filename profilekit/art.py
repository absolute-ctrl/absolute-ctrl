...
# --------------------------------------------------------------------------- http_log
def http_log(x):
    """Access-log scroll for a minimal C http server, with a live req/s counter.

    Same scrolling-window mechanic as `terminal`, restyled: request/response records instead
    of mainframe job output, an occasional accept()/epoll line standing in for CUSTPROC steps.
    """
    P, sid, esc = x.pal, x.sid, x.esc
    step = float(x.opts.get("step_seconds", 0.4))
    rnd = random.Random("httpd-" + sid)
    M, V, lh, top = 16, 6, 14, 90
    methods = ["GET", "GET", "GET", "POST", "GET", "HEAD"]
    paths = ["/", "/index.html", "/api/users", "/api/users/42", "/health", "/static/app.js", "/favicon.ico", "/login"]
    lines, seq = [], 0
    for r in range(M):
        if r % 7 == 6:
            lines.append((f"accept() fd={rnd.randint(9, 30)} worker={r // 7 + 1:02d} backlog={rnd.randint(0, 3)}", "dim"))
        else:
            ok = rnd.random() > .12
            seq += 1
            method, path = rnd.choice(methods), rnd.choice(paths)
            status = "200" if ok else rnd.choice(["404", "500"])
            lines.append((f"{seq:05d} {method:<4} {path:<16} {status}  {rnd.randint(1, 40):>3}ms",
                          "primary" if ok else "accent"))
    rows = ""
    for i in range(M + V):
        text, col = lines[i % M]
        rows += f'<text x="34" y="{top + i * lh}" fill="{P[col]}" font-size="11" xml:space="preserve">{esc(text)}</text>'
    ...
    counters = ""
    for i in range(M):
        n = 1200 + (i + 1) * 331 + rnd.randint(0, 40)
        rate = rnd.randint(800, 4200)
        w = [(i * step, (i + 1) * step)]
        counters += (f'<text x="34" y="184" fill="{P["accent"]}" font-size="11" opacity="0" xml:space="preserve">'
                     f'REQ {n:07d}  {rate / 1000:.1f}K req/s  RC=0000{windows(w, dur)}</text>')

    return (f'<clipPath id="{sid}-term"><rect x="{SX0}" y="{top - 11}" width="{SX1 - SX0}" height="{V * lh}"/></clipPath>'
            f'<rect x="{SX0}" y="{SY0 + 2}" width="{SX1 - SX0}" height="18" fill="{P["shade"]}"/>'
            f'<text x="34" y="59" fill="{P["accent"]}" font-size="11" font-weight="700" xml:space="preserve">{esc(x.opts.get("header", "httpd :: access log"))}</text>'
            f'<text x="34" y="76" fill="{P["dim"]}" font-size="11" xml:space="preserve">{esc(x.opts.get("columns", "SEQ   METHOD PATH             STATUS  MS"))}</text>'
            f'<g clip-path="url(#{sid}-term)"><g>{rows}'
            f'<animateTransform attributeName="transform" type="translate" calcMode="discrete" dur="{f(dur)}s" '
            f'repeatCount="indefinite" keyTimes="{kt}" values="{scroll}"/></g></g>'
            f'<line x1="{SX0}" y1="{top + V * lh - 8}" x2="{SX1}" y2="{top + V * lh - 8}" stroke="{P["dim"]}" stroke-dasharray="2 3"/>'
