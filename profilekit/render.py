...
class Profile:
    ...
    # ------------------------------------------------------------ sections
    def mosaic_rows(self, art, y, row_hold):
        """Prints an image as characters, one row at a time. Returns (height, svg)."""
        ...
        for r, row in enumerate(grid):
            # every glyph gets its own x, so the grid stays square whatever font the browser
            ...
            if spans:
                b += tl.show(f'<text y="{y + r * rh:.1f}" font-size="{size:.2f}" '
                             f'xml:space="preserve">{spans}</text>', hold=row_hold)
            else:
                tl.wait(row_hold)
        return rows_n * rh, b

    def logo_art(self, c, y, row_hold):
        """The picture at the top of a splash: character mosaic, tinted image, or nothing."""
        P, tl, W = self.P, self.tl, self.W
        art = c.get("art") or {}
        ...
        if mode == "image" and art.get("src"):
            ...
            try:
                return ih + 14, tl.show(slot_image.layer(f"logo-{tl.uid('l')}", art, P, ROOT,
                                                         box=((W - iw) / 2, y, iw, ih)), kind="chunk")
            except (OSError, ValueError) as err:
                return 44, tl.show(self.text(self.M, y + 20, f"logo image error: {err}",
                                             P["accent"], self.S["small"]))
        try:
            h, b = self.mosaic_rows(art, y, row_hold)
            return h + 16, b
        except (OSError, ValueError, KeyError) as err:
            return 34, tl.show(self.text(self.M, y + 16, f"logo art unavailable: {err}",
                                         P["dim"], self.S["small"]))

    def logo_panel(self, c, y, name_size=None):
        """Shared composition for the opening splash and the closing login screen:
        picture, brand, tagline, optional fill bar, small centred lines. Returns (bottom_y, svg)."""
        P, tl, W = self.P, self.tl, self.W
        lg = tl.cfg["logo"]
        b = ""
        top_l, top_r = c.get("top_left", ""), c.get("top_right", "")
        if top_l or top_r:
            b += tl.show(self.text(self.M, y, top_l, P["dim"], self.S["small"]) +
                         self.text(W - self.M, y, top_r, P["dim"], self.S["small"], anchor="end"),
                         kind="chunk")
            y += 20

        h, art_svg = self.logo_art(c, y, lg["row_seconds"])
        b += art_svg
        y += h

        size = name_size or self.S["logo_name"]
        if c.get("name"):
            b += tl.show(self.text(W / 2, y + size * 0.8, c["name"], P["accent"], size, weight="700",
                                   anchor="middle", extra=f' letter-spacing="{c.get("name_spacing", 6)}"'),
                         kind="chunk")
            y += size + 10
        if c.get("tagline"):
    ...
    def boot_screen(self):
        """The manufacturer screen, played before anything else and then cleared away.

        It is an overlay across the whole canvas rather than a section, so when it leaves it takes
        its space with it and the profile starts at the top of the image. Phases: black, the tube
        warming up to the phosphor glow, the logo printing, a hold, then black again with nothing
        but a waiting cursor until the BIOS takes over.
        """
        P, tl, W = self.P, self.tl, self.W
        c = self.cfg.get("logo")
        if not c or not c.get("enabled", True) or not tl.on:
            return ""
        black = float(c.get("black_seconds", 1.0))
        warm = float(c.get("warmup_seconds", 0.8))
        warm_hold = float(c.get("warm_hold_seconds", 2.0))
        hold = float(c.get("hold_seconds", tl.cfg["logo"].get("hold_seconds", 5.0)))
        blank = float(c.get("blank_seconds", 1.6))

        veil = (f'<rect width="{W}" height="__H__" fill="{c.get("black", "#000000")}">'
                f'<animate attributeName="opacity" from="1" to="0" begin="{tl.fmt(black)}" '
                f'dur="{warm:.2f}s" fill="freeze"/></rect>')

        tl.t = black + warm + warm_hold   # wall-clock: nothing but the glow until here
        start = tl.now()
        y, panel = self.logo_panel(c, 38)
        if c.get("frame", True):
            panel = self.timed(f'<rect x="{self.M - 12}" y="16" width="{W - 2 * (self.M - 12)}" '
                               f'height="{y + 6}" rx="10" fill="none" stroke="{P["dim"]}" opacity=".45"/>',
                               start) + panel
        tl.hold(hold)
        hide = tl.now()
        cursor = self.timed(self.blink_cursor(float(c.get("cursor_x", self.M + 12)),
                                              float(c.get("cursor_y", 64)), self.S["boot"], begin=hide), hide)
        tl.hold(blank)
        end = tl.now()
        return (f'<g><set attributeName="opacity" to="0" begin="{tl.fmt(end)}" fill="freeze"/>{veil}'
                f'<g><set attributeName="opacity" to="0" begin="{tl.fmt(hide)}" fill="freeze"/>{panel}</g>'
                f'{cursor}</g>')

    def sec_login(self):
        """The closing screen: the helmet again, and a login that never gets answered."""
        P, tl, W = self.P, self.tl, self.W
        c = self.cfg.get("login") or {}
        y, b = self.logo_panel(c, 34, name_size=c.get("name_size", self.S["logo_name"] * 0.62))

        if c.get("title"):
            b += tl.show(self.text(W / 2, y + 18, c["title"], P["accent"], self.S["header"] + 3,
                                   weight="700", anchor="middle"), kind="chunk")
            y += 34

        bw = float(c.get("box_width", 440))
        bx = (W - bw) / 2
        rows = c.get("fields") or [{"label": "login: ", "value": "", "cursor": True}]
        bh = 24 + len(rows) * 30
        b += tl.show(f'<rect x="{bx:.0f}" y="{y}" width="{bw:.0f}" height="{bh}" fill="none" '
                     f'stroke="{P["dim"]}"/>', kind="chunk")
        ry = y + 34
        for row in rows:
            label, value = row.get("label", ""), str(row.get("value", ""))
            if value:
                b += self.typed(bx + 20, ry, value, prompt=label, size=self.S["body"] + 1,
                                prompt_fill=P["primary"], cursor=bool(row.get("cursor", False)))
            else:
                b += tl.show(self.text(bx + 20, ry, label, P["primary"], self.S["body"] + 1), hold=0.05)
                if row.get("cursor", True):
                    b += self.blink_cursor(bx + 20 + len(label) * self.adv(self.S["body"] + 1) + 2, ry,
                                           self.S["body"] + 1)
            ry += 30
        y += bh + 10

        for line in c.get("footer_lines", []):
            b += tl.show(self.text(W / 2, y + 18, line, P["dim"], self.S["small"], anchor="middle"))
            y += 24
        return y + 24, b
    ...
    def sec_telemetry(self):
        ...
        ignored = {str(name).strip().lower() for name in c.get("languages_ignore", [])}
        if ignored:
            langs = {name: size for name, size in langs.items() if name.strip().lower() not in ignored}
    ...
    def build(self):
        ...
        overlay = self.boot_screen()   # runs first: it owns the clock until the profile starts
        ...
        for fn in (self.sec_boot, self.sec_whoami, self.sec_specs, self.sec_mission):
            h, b = fn()
        ...
        login = cfg.get("login")
        if login and login.get("enabled", True):
            if self.FX["dividers"]:
                body += tl.show(self.divider(y + 10), hold=tl.cfg["section_seconds"], kind="chunk")
            y += 20
            h, b = self.sec_login()
            body += f'<g transform="translate(0,{y})">{b}</g>'
            y += h
